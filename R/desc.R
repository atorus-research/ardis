...
get_summaries <- function(e = caller_env()) {
  ...
  summaries <- exprs(
    ...
    iqr     = IQR(.var, na.rm=TRUE, type=getOption('ardis.quantile_type')),
    q1      = quantile(.var, na.rm=TRUE, type=getOption('ardis.quantile_type'))[[2]],
    q3      = quantile(.var, na.rm=TRUE, type=getOption('ardis.quantile_type'))[[4]],
