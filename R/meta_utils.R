#' Extract the result metadata of a tardis table
#'
#' Given a row_id value and a result column, this function will return the
#' tardis_meta object associated with that 'cell'.
#'
...
#' subset data frame.
#'
#' @param x A built tardis table or a dataframe
...
#' @export
get_meta_result.tardis_table <- function(x, row_id, column, ...) {
  m <- x$metadata
...
#'
#' Given a row_id value and a result column, this function will return the
#' subset of data referenced by the tardis_meta object associated with that
...
#' @param target A data frame to be subset (if not pulled from a tardis table)
#' @param ... additional arguments
#'
#' @return A data.frame
...
get_meta_subset.data.frame <- function(x, row_id, column,
  ...
  if (is.null(target)) {
    stop("If querying metadata without a tardis_table, a target must be provided", call.=FALSE)
  }
...
#' @export
#' @rdname get_meta_subset
get_meta_subset.tardis_table <- function(x, row_id, column, add_cols = vars(USUBJID), ...) {
